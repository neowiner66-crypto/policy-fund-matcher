...
from core.matching_engine import calculate_matches, merge_active_sources, filter_by_region

st.set_page_config(page_title="매칭 결과 · 정책자금 매칭 어시스턴트", page_icon="📌", layout="wide")
...
st.title(f"매칭 결과 — {company['name']}")
st.write(
    "정책자금과 지원사업 공고 중 **잠재적으로 관련 있는 후보를 넓게 찾아드리는 초기 탐색 결과**입니다. "
...
# 정책자금(policy_fund)은 지역 무관 항상 유지됨 (filter_by_region 참고).
...
c4.metric("자금목적", f"{company['purpose']}자금")
c5.metric("필요자금", f"{company['needed_amount_eok']:g}억원")
c6.metric("체크된 자가진단", f"{total_checked} / 55")

st.warning(
    "**점수가 높다고 자동 승인이 아닙니다.** 아래 순위는 자격요건·자금목적·강점신호를 기준으로 한 "
...
subtitle = f"추천 후보 {len(results)}건 (정책자금 {len(results) - n_announcements}건 · 지원공고 {n_announcements}건"
subtitle += f", 그중 마감됨 {n_expired}건)" if n_expired else ")"
st.subheader(subtitle)
region_note = f" · {company.get('region_sido', '')} 지역과 무관한 공고 {_n_excluded_by_region}건은 제외(전국 대상 공고는 포함)" if _n_excluded_by_region else ""
st.caption(f"점수 = 자격요건 충족 여부 + 자금목적 부합도 + 업종/강점신호 가중합 (참고용){region_note}")
...
KIND_LABEL = {"policy_fund": "🏦 정책자금", "announcement": "📢 지원공고"}
...
for r in results[:display_count]:
    ...
    with st.container(border=True):
        ...
        with col_main:
            st.caption(badge)
            st.markdown(f"**{fund['name']}**  \n<span style='color:#8590a0;font-size:0.85em'>{institution}</span>", unsafe_allow_html=True)
...
st.divider()
st.caption("* 최신 금리·한도·접수기간은 각 기관 공식 공고에서 다시 확인하세요.")
