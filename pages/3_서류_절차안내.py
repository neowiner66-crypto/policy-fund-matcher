"""기능 ③ 추천 자금의 필요서류·접수처·유의사항 안내 + 상담 결과 저장."""
import json
...
st.set_page_config(page_title="서류·절차 안내 · 정책자금 매칭 어시스턴트", page_icon="📌", layout="wide")
...
if not selected_fund_id:
    st.warning("매칭 결과에서 안내받을 자금을 먼저 선택해주세요.")
    if st.button("← 매칭 결과로 이동"):
...
if fund is None:
    st.error("선택한 자금 정보를 찾을 수 없습니다. 매칭 결과에서 다시 선택해주세요.")
    if st.button("← 매칭 결과로 이동"):
        st.switch_page("pages/2_매칭결과.py")
    st.stop()
...
c1.metric("자금유형", fund.get("type", "-"))
c2.metric("한도", fund.get("limit_text", "-"))
...
if fund.get("kind") == "announcement":
    ...
    if fund.get("source_url"):
        st.markdown(f"🔗 [공고 원문 바로가기]({fund['source_url']})")

if selected_result and selected_result.get("reasons"):
    with st.expander("이 자금이 추천된 이유"):
...
st.warning(
    "⚠️ **면책 안내**\n\n"
    "- 금리·한도·접수기간은 변경될 수 있으므로 신청 시점 공식공고를 반드시 최종 확인하세요.\n"
    "- 본 서비스는 승인을 보장하지 않습니다. 최종 결정은 각 기관의 심사 결과에 따릅니다."
)

st.divider()
col_back, col_save = st.columns([1, 1])
with col_back:
    if st.button("← 매칭 결과로 돌아가기"):
        st.switch_page("pages/2_매칭결과.py")
