...
ML/AI 없이 if-then 규칙 + 가중치 점수표로 구현한다 (CLAUDE.md 2절 참고).

절차:
  1차: 자금목적 적합도로 후보를 넓게 잡는다 (완전 배제하지 않고 감점 처리 —
       "자격요건 통과 = 선정 확정 아님" 원칙을 지키기 위해 하드 필터링을 최소화).
  2차: 업력·지역·자가진단 체크리스트(강점신호)로 가중치 점수를 더한다.
  3차: 점수순으로 정렬한다.

주의: 점수는 어디까지나 참고용 우선순위이며, 실제 승인 가능성을 보장하지 않는다.
"""
...
# (기업마당은 전국 공고에 지역 태그를 아예 안 붙이는 대신, 16개 지역 태그를 전부 붙이는 방식을 씀)
...
def filter_by_region(funds_data: Dict, sido: Optional[str]) -> Dict:
    ...
    정책자금(kind="policy_fund")은 원래 지역 무관하게 항상 통과시킨다 — 지자체 자금은
...
def merge_active_sources(
    ...
    """정책자금(funds_master.json) + 지원사업 공고(announcements.json)를 하나의 후보 목록으로 합친다.
    ...
    포함된 각 항목에는 is_expired(bool)가 추가로 표시된다 (정책자금은 항상 False).
...
def _checklist_category_counts(checklist_answers: Dict[str, bool], checklist_data: Dict) -> Dict[str, Dict]:
    ...
    for cat in checklist_data["categories"]:
        ...
        result[cat["key"]] = {"checked": checked, "total": total, "ratio": ratio}
    return result
...
def calculate_matches(
    ...
    """기업정보 + 자가진단 체크리스트를 기준으로 정책자금 후보를 점수순으로 정렬한다.
    ...
    debt_eok = float(company.get("debt_eok") or 0)
    ...
    for fund in funds_data["funds"]:
        ...
        if purpose in fund.get("eligible_purposes", []):
            ...
            reasons.append(f"자금목적 부합: {purpose}자금")
        else:
            score -= PURPOSE_MISMATCH_PENALTY
            reasons.append("자금목적 부합도 낮음 — 별도 확인 필요")
        ...
        if fund.get("region_required"):
            if region:
                ...
                reasons.append(f"{region} 지역 소재 — 지자체 협약자금 대상")
            else:
        ...
        for cat_key in fund.get("relevant_categories", []):
            ...
            if info["checked"] > 0:
                reasons.append(f"'{cat_key}' 자가진단 {info['checked']}/{info['total']} 항목 해당")
        ...
        # 2차: 담보 부족 보완 신호 (보증기관 매칭에 활용)
        if fund.get("debt_bonus") and debt_eok > 0:
            score += fund["debt_bonus"]
            reasons.append("기존 차입 보유 — 신용보증으로 담보력 보완 가능")

        # 지원사업 공고는 정책자금과 달리 접수기간이 있으므로 마감일을 항상 눈에 띄게 표시
